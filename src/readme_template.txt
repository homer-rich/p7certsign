...
1)  Determine the Thumbprint on DoD Root CA 3 (the first SIGNINGCHAIN certificate) using the following command:
openssl x509 -in SIGNINGCHAIN -subject -issuer -fingerprint -noout
...
3) Verify that only DoD Root 3 is present in the SIGNINGCHAIN file using the following command:
openssl crl2pkcs7 -nocrl -certfile SIGNINGCHAIN | openssl pkcs7 -print_certs -noout
...
4) Verify the S/MIME signature on IRFILENAME.sha256 using the following command:

On Mac OSX:
$ openssl smime -verify -in IRFILENAME.sha256 -inform DER -CAfile SIGNINGCHAIN | shasum -a 256 -c
...
On Linux:
$ openssl smime -verify -in IRFILENAME.sha256 -inform DER -CAfile SIGNINGCHAIN | dos2unix | sha256sum -c
...
> openssl smime -verify -in IRFILENAME.sha256 -inform DER -CAfile SIGNINGCHAIN
...
Verify that the output matches the hash listed for the file in the IRFILENAME.sha256 openssl smime -verify output.
...
openssl pkcs7 -in IRFILENAME_pem.p7b -print_certs -out DoD_CAs.pem
